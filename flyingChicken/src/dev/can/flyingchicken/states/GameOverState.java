...
public class GameOverState extends State {
    ...
    public GameOverState(Game game) {
        ...
        if (entityManager.getChicken().getPoint() < 300) {
            sky = ImageLoader.loadImage("/img/morning.png");
        } else if (entityManager.getChicken().getPoint() < 900) {
            sky = ImageLoader.loadImage("/img/noon.png");
        } else {
    ...
    public void render(Graphics g) {
        ...
        if (timer <= 100) {
            g.drawImage(Assets.label_over, game.getWidth()/2 - 120, game.getHeight()/2 - 120, 240, 240, null);
        } else {
            g.drawImage(Assets.label_try, game.getWidth()/2 - 120, game.getHeight()/2 - 120, 240, 240, null);
        }
    }
}
